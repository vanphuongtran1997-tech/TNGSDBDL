...
import { Student, ClassRoom, GradeRecord, AttendanceRecord, ConductRecord, TuitionItem, AttendanceStatus } from '../types';
import { 
...
export const StudentIdSearchModal: React.FC<StudentIdSearchModalProps> = ({
  ...
  const sampleIds = useMemo(() => {
    return students.slice(0, 6).map(s => s.id);
  }, [students]);
  ...
  }, [students, searchIdQuery]);

  // Active student object
  const activeStudent = useMemo(() => {
    return students.find(s => s.id === selectedStudentId) || matchedStudents[0] || students[0] || null;
        ...
        <div className="p-4 bg-slate-50 border-b border-slate-200 space-y-3">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Nhập mã học sinh để tìm kiếm (vd: DBS-KT-001, 001, KT-001, hoặc họ tên)..."
